 ...
 * Hardware Feature:
 * 1. GE Rot1 (任意角度硬件旋转)
 * 2. GE Scaler (全屏硬件拉伸) - 配合 Over-Scaling 消除边缘死角
 * 3. GE_PD_ADD (Porter/Duff Rule 11: 硬件加法混合)
 * 4. GE FillRect (双重硬件背景清理：屏幕 + 中间缓冲区)
 * 覆盖机能：此特效通过“多重清理-旋转-深度缩放-加法混合”流水线，实现了绝对纯净的全屏干涉视觉。
 */
...
static void effect_draw(struct demo_ctx *ctx, unsigned long phy_addr)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        rot_fill.dst_buf.stride[0]   = TEX_W * 2;
        rot_fill.dst_buf.size.width  = TEX_W;
        rot_fill.dst_buf.size.height = TEX_H;
        rot_fill.dst_buf.format      = MPP_FMT_RGB_565;
        ...
        // 计算角速度与相位
        int theta     = (g_tick * (i + 1) + (i * 256)) & 511;
        ...
        rot.ctrl.alpha_en    = 1; // 极性定义：1 禁用混合
         ...
         */
        int crop_w              = 180 + (GET_SIN(g_tick << 1) >> 8); // 缩小采样范围，实现强制放大
